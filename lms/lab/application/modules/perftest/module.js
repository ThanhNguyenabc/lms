 ...
 try {
	...
	for(var i = 0; i<actions; i++)
	{
		...
		case 0:
			...
			await Client_Wait((250 + Numbers_Random(0, 1000)) / 1000);
			}
		break;
 ...
 for (let a = 0; a < actions; a++) {
	try {
		...
		while (classes.length == 0) {
			...
			var label = UI_Element_Find(labels[key]);

			Document_Event_Trigger(label, "click");
			await Client_Wait((1000 + Numbers_Random(0, 1000)) / 1000);
		...
		await Client_Wait((5000 + Numbers_Random(0, 1000)) / 1000);
							...
							switch (caption.textContent) {
								case "HOMEWORK": case "VOCABULARY":
									...
									while (checkfinishtest.style.display == "none"){
										...
										await Client_Wait((250 + Numbers_Random(0, 500)) / 1000);
									}
									
									//Document_Event_Trigger(checkfinishtest, "click");
									await Client_Wait((500 + Numbers_Random(0, 1000)) / 1000);
								...
								default:
									...
									if(typeof playbutton !== "undefined"){
										...
										await Client_Wait((1000 + Numbers_Random(0, 500)) / 1000);
										Document_Event_Trigger(playbutton, "click");

										await Client_Wait((10000 + Numbers_Random(0, 1000)) / 1000);
									} 
									else await Client_Wait((3000 + Numbers_Random(0, 1000)) / 1000);
									break;
							}
						} catch (error) {
							await Client_Wait((500 + Numbers_Random(0, 1000)) / 1000);
      ...
      if(dateEnd - date_start > 0  && date_start - dateStart > 0) 
      {
        ...
        while (dataIndex < data.length) {
          // EXTRACT A CLASS AND LOAD IT
            ...
            Core_State_Set("global", ["view-class"], cls["id"]);
            await Module_Load("classroom", UI_Element_Find("main-module"));
            await Client_Wait((1000 + Numbers_Random(0, 2500)) / 1000);
